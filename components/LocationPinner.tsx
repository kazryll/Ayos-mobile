...
const LocationPinner: React.FC<LocationPinnerProps> = ({
  onLocationConfirm,
  onBack,
}) => {
  ...
  const getCurrentLocation = async () => {
    try {
      ...
      if (status !== "granted") {
        setError(
          "Location permission is required to pin the issue location. Please enable location permissions in your device settings."
        );
        setLoading(false);
        return;
      }
  ...
  const reverseGeocode = async (lat: number, lng: number) => {
    try {
      console.log(
        "📍 Starting reverse geocoding via Google Maps API for:",
        lat,
        lng
      );
      ...
      if (data.results && data.results.length > 0) {
        ...
        setSelectedLocation((prev) =>
          prev ? { ...prev, address: fallbackAddress } : prev
        );
      }
    } catch (error) {
    ...
    return (
      <View style={styles.mapFallback}>
        ...
        <View style={styles.fallbackLocation}>
          ...
          {selectedLocation ? (
            <>
              <Text style={styles.fallbackLocationAddress}>
